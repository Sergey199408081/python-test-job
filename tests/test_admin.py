...
async def test_admin_get_me_success(client):
    ...
    _, resp = await client.get("/api/admin/me", headers={"Authorization": f"Bearer {token}"})
...
async def test_admin_list_users(client):
    token = await get_admin_token(client)
    _, resp = await client.get("/api/admin/users", headers={"Authorization": f"Bearer {token}"})
    assert resp.status == 200
    data = resp.json
...
async def test_admin_create_user(client):
    token = await get_admin_token(client)
    _, resp = await client.post("/api/admin/users", json={
...
async def test_admin_update_user(client):
    token = await get_admin_token(client)
    _, create_resp = await client.post("/api/admin/users", json={
    ...
    _, resp = await client.put(f"/api/admin/users/{user_id}", json={
...
async def test_admin_update_user_nonexistent(client):
    token = await get_admin_token(client)
    _, resp = await client.put("/api/admin/users/99999", json={
        "full_name": "Ghost",
    }, headers={"Authorization": f"Bearer {token}"})
...
async def test_admin_delete_user(client):
    ...
    _, resp = await client.delete(f"/api/admin/users/{user_id}", headers={"Authorization": f"Bearer {token}"})
    assert resp.status == 200
...
async def test_admin_delete_user_nonexistent(client):
    token = await get_admin_token(client)
    _, resp = await client.delete("/api/admin/users/99999", headers={"Authorization": f"Bearer {token}"})
    assert resp.status == 404
