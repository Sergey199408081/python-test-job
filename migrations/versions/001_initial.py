...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('balance', sa.Numeric(precision=15, scale=2), default=Decimal('0.00'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now()),
    ...
    op.create_table(
        'payments',
        sa.Column('id', sa.Integer(), primary_key=True, index=True),
        sa.Column('transaction_id', sa.String(36), unique=True, index=True, nullable=False),
